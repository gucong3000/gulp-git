...
module.exports = function (message, opt) {
	...
	const messageEntry = function (entry) {
		return '-m "' + entry + '" ';
	};

	const flush = function (cb) {
		let writeStdin = false;
		let cmd = 'git commit ';
		...
		const execChildProcess = exec(cmd, opt, (err, stdout, stderr) => {
			if (err && (String(stdout).indexOf('no changes added to commit') === 0)) {
				return cb(err);
			}
			if (!opt.quiet) {
